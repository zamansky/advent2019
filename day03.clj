...
(defn add-to-map [m  point [dir dist] id ]
...
(defn solve [input]
  (let [l1 (build-map {} (parse-line (input 0)) 0)
        ...
        ans (first (sort  (map (fn [e] (+ (abs ((e 0) 0)) (abs ((e 0) 1))))
                               (filter (fn [e] (= (count (e 1)) 2)) (dissoc l2 :point)))))
        ]
    ans
    ))

(defn build-ints [input]
  (let [l1 (build-map {} (parse-line (input 0)) 0)
        l2 (build-map l1 (parse-line (input 1)) 1)
        ints-list(filter (fn [e] (= (count (e 1)) 2)) (dissoc l2 :point))
        ints (apply merge (map (fn [e] (into {} {(e 0) 0})) ints-list))
        ]
    ints
...
(def dists (build-ints testlines))
